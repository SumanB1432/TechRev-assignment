  ...
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res.status(400).send({
        status: false,
        message: "Please give some data to create customer",
      });
    }
    if (!isValid(fname)) {
      return res
        .status(400)
        .send({ satus: false, message: "Please provide your first_name" });
    }
    if (!fname.trim().match(/^[a-zA-Z]+$/)) {
      return res
        .status(400)
        .send({ status: false, message: "please provide a valid first_name" });
    }

    if (!isValid(lname)) {
      return res
        .status(400)
        .send({ satus: false, message: "Please provide your last_name" });
    }
    if (!lname.trim().match(/^[a-zA-Z]+$/)) {
      return res
        .status(400)
        .send({ status: false, message: "please provide a valid last_name" });
    }
    ...
    if (!isValid(phone)) {
      return res
        .status(400)
        .send({ status: false, Message: "Please provide your phone number" });
    }
...
const listOfCustomer = async function (req, res) {
  try {
    ...
    if (list.length == 0) {
      return res
        .status(404)
        .send({ status: false, message: "no customer present" });
    }
    let fullList = [];
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      let address = await addressModel
        ...
        .select({ city: 1, state: 1, dist: 1, pincode: 1 });
      fullList.push({
        fname: list[i].fname,
        lname: list[i].lname,
        email: list[i].email,
...
const getCustomerById = async function (req, res) {
  try {
    ...
    if (!data) {
      return res
        .status(404)
        .send({ status: false, message: "No such customer exist" });
    }
    let address = await addressModel
...
const deleteCustomer = async function (req, res) {
  try {
    ...
    if (!mongoose.isValidObjectId(customerId)) {
      return res
        .status(400)
        .send({ status: false, message: "please provide a valid customerID" });
    }
    let find = await customerModel.findOne({
      _id: customerId,
      isDeleted: false,
    });
    if (!find) {
      return res.status(404).send({
        status: false,
        message: `${customerId} is already deleted or not exist`,
      });
    }
    let del = await customerModel.findByIdAndUpdate(customerId, {
      isDeleted: true,
    });
    let deleteinAddress = await addressModel.updateMany(
      { customerId: customerId, isDeleted: false },
      { isDeleted: true }
    );
    return res
      .status(200)
      .send({ status: false, message: "Deleted successfully" });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
};
...
const updateCustomer = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length == 0) {
      return res.status(400).send({
        status: false,
        message: "Please provide data to update details",
      });
    }
    ...
    if (!customerId) {
      return res
        .status(400)
        .send({ status: false, status: "please procide customerId" });
    }
    if (!mongoose.isValidObjectId(customerId)) {
      return res
        .status(400)
        .send({ status: false, message: `${customerId} is a invalid Id` });
    }

    let getCustomer = await customerModel.findOne({
      _id: customerId,
      isDeleted: false,
    });
    if (!getCustomer) {
    ...
    if (data.fname != undefined) {
      if (
        ...
        !data.fname.trim().match(/^[a-zA-Z]+$/)
      ) {
        return res.status(400).send({
          status: false,
          message: "please provide a valid first_name",
        });
      }
    }
    ...
    if (data.email != undefined) {
      if (
        ...
        !data.email.trim().match(emailRegex)
      ) {
        return res
          .status(400)
          .send({ status: false, message: "please provide a valid email" });
      }
      let getEmail = await customerModel.findOne({
        email: data.email,
        isDeleted: false,
      }); //CHECK DUPLICASY
    ...
    if (data.password != undefined) {
      if (
        ...
        return res.status(400).send({
          status: false,
          message:
            "please provide a valid password/Please provide a valid password ,Password should be of 8 - 15 characters",
        });
      }
    }
    ...
    if (data.phone != undefined) {
      if (
        ...
        !phone.trim().match(phoneRegex)
      ) {
        return res
          .status(400)
          .send({ status: false, message: `${data.phone} is not valid` });
      }
      let getPhone = await customerModel.findOne({
        phone: data,
        phone,
        isDeleted: false,
      }); //CHECK DUPLICASY
