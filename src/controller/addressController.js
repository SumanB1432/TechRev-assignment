...
const createAddress = async function (req, res) {
  try {
    ...
    if (!regex.test(city)) {
      return res
        .status(400)
        .send({ status: false, message: "please insert a valid city" });
    }

    if (!isValid(dist)) {
      return res
        .status(400)
        .send({ status: false, message: "please provide dist" });
    }

    if (!isValid(state)) {
      return res
        .status(400)
        .send({ status: false, message: "please provide state" });
    }
    ...
    if (!checkCustomer) {
      return res
        .status(404)
        .send({ status: false, message: "Ths customer is not exist" });
    }
    if (!mongoose.isValidObjectId(customerId)) {
      return res.status(400).send({
        status: false,
        message: `${customerId} is a invalid customerID`,
      });
    }
...
const addressUpdate = async function (req, res) {
  try {
    ...
    if (!mongoose.isValidObjectId(addressId)) {
      return res
        .status(400)
        .send({ status: false, message: `${addressId} is not valid` });
    }
    ...
    let getAddress = await addressModel.findOne({
      _id: addressId,
    ...
    if (!getAddress) {
      return res
        .status(404)
        .send({ status: false, message: "No such address exist" });
    }
    ...
    if (data.city != undefined) {
      if (!isValid(data.city) || typeof data.city != "string") {
        return res
          .status(400)
          .send({ status: false, message: "Please provide a valid city " });
      }
    }
    if (data.dist != undefined) {
      if (!isValid(data.dist) || typeof data.dist != "string") {
        return res
          .status(400)
          .send({ status: false, message: "Please provide a valid distict" });
      }
    }
    if (data.state != undefined) {
      if (!isValid(data.state) || typeof data.state != "string") {
        return res
          .status(400)
          .send({ status: false, message: "please provide a valid state" });
      }
    }
    if (data.pincode != undefined) {
      if (
        !isValid(data.pincode) ||
        typeof data.pincode != "number" ||
        !pinRegex.test(data.pincode)
      ) {
        return res
          .status(400)
